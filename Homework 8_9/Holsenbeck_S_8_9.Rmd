...
<blockquote>*Original* $H_1$ Is the proportion of individuals whom qualify as ?
...
<li>Try to find an example where a variable appears signficant in the bivariate regression, but not in the full regression. Is this an example of a spurious or a chained causal pathway?</li>
</ol>
</div>
<div class="a">
## a
...
This coefficients have dropped in value, as attempting to make a correlation between independent variables that might have a more direct effect (assuming there is a correlation between prevalence risk factors and Republicanism in a state) are included with the health variable act300, which is inversely correlated in the bivariate regressions. We can see that including all three of these variables has also mitigated the negative correlaton with act300 and actually made it positive. 
## b
It can be inferred, that were variables with all positive correlations chosen for the multivariate regression, are more pronounced effect will be observed. To test this, we can add <u>act0</u> - "Percent of adults who engage in no leisure-time physical activity" to the model. It is expected that due to the likely multicolinearity between this variable and <u>wgtObs</u>, we will observed larger coefficients, increased significance, but also a high VIF. To test:
```{r '8b'}
...
<br>
<p>By evaluating the variables, it's apparent that wgtObs and act300 are going to be mutually exclusive, and likely going to dampen the coefficients and significance of one another when included in the same model. We can confirm this by adding all variables that are likely to all be correlated:</p>
```{r '8c - cont'}
R0VegObs <- lm(R~act0+conVeg+wgtObs,Pty_Hlth)
vif(R0VegObs)
summary(R0VegObs)
stargazer(R0VegObs, no.space=TRUE, dep.var.labels=c("% identified as Republican"), 
          omit.stat=c("ll"),header=FALSE,type="html")
```

<p>
The overall p-value for the model is now statistically significant, as well as the coefficient for wgtObs. The coefficients are higher, and the $R^2$ is similar to other models. What still remains confusing is the negative coefficient for act0, which when regressed with wgtObs and conVeg, and R individually, are all positive.
```{r}
...
None of which are reflected in the multivariate regression, likely due to the reasons mentioned in the previous answer regarding inverse relationships and resulting dampening of the coefficient values. There may be confounding variables between conFru and wgtObs, and all risk factors or healthy behaviors were they grouped together, that being overall health knowledge, access to health information etc. as discussed prior. There is definite mutual exclusivity between wgtObs and act300, which isn't spurious or chained but important to note. It could logically be conceived that act0 and wgtObs would be chained in causation, but the results from previous answers make this relationship difficult to discern.
