...
```
```{r 'Use Google Sheets to Calculate Chi Squared by hand', eval=F}
#Use Google Sheets to Calculate the Chi Squared 'by hand'
install.packages("googlesheets")
library(googlesheets)
#Authorize a northeastern acct to have access to the sheet below
gs_auth()
#register a google sheet object with the 
...
```{r 'Test values for fn from class example',eval=F}
#Here are values from the example at http://nickbeauchamp.com/comp_stats_NB/compstats_06-01.html
#to test and verify the functionality of the custom f(n) below with
N <- c(91L,111L,74L)
M <- c(3.23L,3.9L,4.7L)
...
<div class="a">A)The results of the F-Test with the summary data provided indicate that we fail to reject the null, and conclude that the average age is the same across the party affiliations.</div>
...
<div class="a">A)The results from the simulated data experiment do not match the results from 2a. When we take a look at the actual data from the simulation (See 2nd chunk below) we can see consistency in the data between the input vectors and the dataframe used in the experiment, however, when we compare the actual mean and sd of the datasets, to the mean & sd inputs for the rnorm function, we can see that these values are slightly different. The difference between the actual experimental data mean & sd, and the summary mean & sd used in the calculations likely accounts for the differing outcome between the anova test and the manual F test. This would suggest that a savvy statistician should avoid using summary data for inputs and always run the test with the actual data to avoid significant accumulated error that results in different test outcomes.</div>
...
#Create a new Worksheet names 2b
...
c(mean(D[,1]),mean(I[,1]),mean(R[,1]))
mu
c(sd(D[,1]),sd(I[,1]),sd(R[,1]))
sd
```
