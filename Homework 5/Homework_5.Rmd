...
tCrit <- function(cl,data,tail,mu=0){
  ...
  return(cValues)
   ...
   ci.d <- f.cI.d(cV,data)
   ts.d <- f.TS.d(data)
   pv.d <- 2*f.pV.d(-1*abs(ts.d),data)
   lc <- -1*qt(cV,length(data)-1)
  hC <- qt(cV,length(data)-1)
  cValues <- tibble::tribble(
    ~Variable,~Values,
    'Low tCrit', lc,
    'High tCrit', hC,
    ...
    if(tail=="L"|tail=="l"){
    cV <- (1-cl)
    lc <- qt(cV,length(data)-1)
  ci.d <- f.cI.d(cV,data)
  ts.d <- f.TS.d(data)
  pv.d <- f.pV.d(ts.d,data)
 cValues <- tibble::tribble(
    ~Variable,~Values,
    'Low tCrit', lc,
    'Conf_Int.Low', ci.d[1],
...
<div class="a">A)If $t_{stat}>t_{crit}$ for a right tailed test then the null hypothesis can be rejected. The calculation below indicates the null can be rejected in a right tailed test at the 95% confidence level.
...
$CI=100\pm 4.3648$  
$CI=\text{Low }t_{Crit}=-1.98421695,\text{High }t_{Crit}=1.98421695$  
```{r '1h'}
tCrit(.95,c(104,100,22,100),"2")
```
...
$df_{s1=s2}=n_1+n_2-2$  
$df_{s1=s2}=50+50-2$
If we then use a two tailed test (the t.2 function does), the confidence interval will be between $low_{Crit}	-1.984467e+00,hi_{Crit}	1.984467e+00$ and we know the t-statistic is -3.025, well beyond the threshold for the rejection range, indicating a statistically significant difference.
</div>
```{r '2a'}
qt(.975,98)
...
<div class="a">A)At the 95% confidence level on a two tailed test the null hypothesis can be rejected in favor of the research hypothesis. It can be concluded that at the 95% confidence level there is a statistically signicant difference between the Tetris skills of males v females (with females outperforming males.</div>
...
<div class="a">A)In this example, I think the standard deviations are different enough where it would be misleading to pool them. We can first compute the $t_{stat}$:  
...
<em style="color:red">To make the findings more interesting for this question, this is where the custom data sets come into use.</em>
```{r '4b'}
...
#Find Total #s of respondents
(Total.Resp <- GSS %>% 
    group_by(YEAR) %>% 
    summarise(n=n())) 
#Group those who answered the question by year, add in columns for mean, sd, n summary stats
(health_by_yr <- GSS %>% group_by(YEAR) %>% 
  filter(HEALTH>0 & HEALTH<5) %>% 
  summarise(mHealth = mean(HEALTH),
            sHealth = sd(HEALTH),
            nHealth = n()))  
#Combine the two  and add percent responding column
health_by_yr <- cbind(health_by_yr,total.n=Total.Resp$n)
health_by_yr <- health_by_yr %>% 
...
The 2 sample t.test $p_{value}$ indicates that we reject the null and that the average response of responding participants (2012~66% & 2016~66% of total) is different at the 95% confidence level. We conclude that at the 95% confidence level, the average respondent indicates that their health is different, and better in 2016 than in 2012.</div>
...
# Create a df with the data in question
(sci.s <- GSS %>%
  filter(SCISTUDY>0) %>% 
  group_by(ID_) %>%
    select(ID_,SCISTUDY,ODDS1,EARTHSUN))
# f(n) For switching answer 1 and 2 on EARTHSUN such that 1 is the wrong answer, as in ODDS1, so that the data can be compared via the paired t-test
switch <- function(x){
  if(x==1){x <- 2}else{x <- 1}
}
#Create a df with those who indicate they have a general sense or clear understanding of scientific findings, switch value 1&2 for EARTHSUN in new col
sci.hi <- sci.s %>% 
...
$t_{stat}=0.01847$  
Given the $t_{crit}$ is ~1.984, we can see that the $t_{stat}<t_{crit}$ thus the sample mean is nowhere near close to significantly different than the expected mean, which fails to reject the null hypothesis. Considering the sample data is a random distribution of 100 numbers on the t-distributon, we would hope that the mean would be near 0.
</div>
```{r '4d'}
mean(data)
sd(data)
