...
```
```{r '1'}
Rep_AgeRacIncIde <- glm(vote_rep~age+race_white+income+ideology_con,family = "binomial",anes)
sjt.glm(Rep_AgeRacIncIde)
sjt.lm(Rep_AgeRacIncIde,digits.est = 3)
```
...
```{r '1a'}
Repint_rWhAgeIncIde <- glm(vote_rep~age*race_white*income*ideology_con,family="binomial",anes)
...
(Wh <- predict.glm(Rep_AgeRacIncIde,input,type="response"))
(Whodds <- predict.glm(Rep_AgeRacIncIde,input))
```

A white person of average age, income, and ideology has a probability of .49 of voting Republican, or about a 50/50 chance. This is consistent with the odds ratio of 1 in the linear model with interaction terms.</div>
### b
<div class="q">
...
```{r '1b'}
input$race_white <- 0
(`dp_BtoWh`<- Wh-(nWh <- predict.glm(Rep_AgeRacIncIde,input,type="response")))
(`dodds_BtoWh` <- Whodds/predict.glm(Rep_AgeRacIncIde,input,type="terms"))
```
The change in probability is +~.41 between a black person and a white person going to vote Republican, or in terms of odds, a white person is ~5.68 times more likely to vote Republican than a black person when age, income and ideology are accounted for.
</div>
### c
<div class="q">
...
$$\begin{aligned}
\frac{P(y=1|x=a)}{1-P(y=1|x=a)}-\frac{P(y=1|x=b)}{1-P(y=1|x=b)} = \\ e^{\beta_0} (e^{\beta_1})^{a_1}(e^{\beta_2})^{a_2}(e^{\beta_3})^{a_3}(e^{\beta_4})^{a_4} - e^{\beta_0} (e^{\beta_1})^{b_1}(e^{\beta_2})^{b_2}(e^{\beta_3})^{b_3}(e^{\beta_4})^{b_4} \\ e^{-8.077} (e^{0.005})^{46.451}(e^{2.437})^{1}(e^{0.405})^{2.704}(e^{1.042})^{4.0978} - \\ e^{-8.077} (e^{0.005})^{46.451}(e^{2.437})^{0}(e^{0.405})^{2.704}(e^{1.042})^{4.0978}
\end{aligned}$$
Again using R to solve...


```{r '1c - Odds effect'}
exp(Rep_AgeRacIncIde$coefficients)
(`1cdodds_BtoWh` <- (exp(b_yint)*exp(b_age)^input[2]*exp(b_rWh)^1*exp(b_inc)^input[3]*exp(b_ide)^input[4])/(exp(b_yint)*exp(b_age)^input[2]*exp(b_rWh)^0*exp(b_inc)^input[3]*exp(b_ide)^input[4]))
#or 
exp(b_rWh)^1/exp(b_rWh)^0
```
The calculation by hand indicates an 11.43x higher chance of voting Republican if one is white v black given all other variables remain constant. These results are inconsistent with those given by the predict.glm function, and I'm not sure why.
...
dinc_lw[3] <- input[3]-1
dinc_hi[3] <- input[3]+1
(`50yrs` <- predict.glm(Repint_rWhAgeIncIde,dage_hi,type="response")-predict.glm(Repint_rWhAgeIncIde,dage_lw,type="response"))
(inc_1 <- predict.glm(Repint_rWhAgeIncIde,dinc_hi,type="response")-predict.glm(Repint_rWhAgeIncIde,dinc_lw,type="response"))
inc_1/`50yrs`
```
...
dinc_lw[1] <- 1
dinc_hi[1] <- 1
(`50yrswh` <- predict.glm(Repint_rWhAgeIncIde,dage_hi,type="response")-predict.glm(Repint_rWhAgeIncIde,dage_lw,type="response"))
(inc_1wh <- predict.glm(Repint_rWhAgeIncIde,dinc_hi,type="response")-predict.glm(Repint_rWhAgeIncIde,dinc_lw,type="response"))
inc_1wh/`50yrswh`
```
...
<div class="a">
```{r '1e'}
Rep_AgeRacGenEduIncIdePid <- glm(vote_rep~age+race_white+gender_male+education+income+ideology_con+partyid_rep,family = "binomial",anes)
